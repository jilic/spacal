...
void LXeEventAction::BeginOfEventAction(const G4Event* anEvent){
	...
	G4EventManager::GetEventManager()->SetUserInformation(new LXeUserEventInformation);
...
void LXeEventAction::EndOfEventAction(const G4Event* anEvent){
	...
	if(fSaveThreshold&&eventInformation->GetPhotonCount() <= fSaveThreshold)G4RunManager::GetRunManager()->rndmSaveThisEvent();

	if(fRecorder)fRecorder->RecordEndOfEvent(anEvent);

	WriteToFile(true, eventInformation);

}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

void LXeEventAction::WriteToFile(bool write, LXeUserEventInformation* evtInfo){

	
	// write output file: optical.dat with 7 columns: 
	// (1) # of generated scint phot., (2) # of generated Ceren phot., 
	// (3) tot # of generated optical phot., (4) # of optical phot. detected 
	// by PMT, (5) # of optical phot. absorbed by PMT, (6) # of optical phot. 
	// absorbed in the scint., (7) # of optical phot. unaccounted for in the event 

//	op << fixed << right;
	op << std::setw(9) << std::setprecision(2) <<  evtInfo->GetPhotonCount_Scint()<<", "
		<< std::setw(9) << std::setprecision(2) << evtInfo->GetPhotonCount_Ceren()<<", "
		<< std::setw(9) << std::setprecision(2) << evtInfo->GetPhotonCount()<<", "
		<< std::setw(9) << std::setprecision(2) << evtInfo->GetHitCount()<<", "
		<< std::setw(9) << std::setprecision(2) << evtInfo->GetBoundaryAbsorptionCount()<<", "
		<< std::setw(9) << std::setprecision(2) << evtInfo->GetAbsorptionCount()<<", " 
		<< std::setw(9) << std::setprecision(2) <<(evtInfo->GetPhotonCount_Scint() + 
				evtInfo->GetPhotonCount_Ceren() - 
				evtInfo->GetAbsorptionCount() -
				evtInfo->GetHitCount() - 
				evtInfo->GetBoundaryAbsorptionCount()) << std::endl; 
